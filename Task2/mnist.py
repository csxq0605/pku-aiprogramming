...
def run(parrallel, trainloader, testloader, classes):
    net = LeNet()
    if parrallel:
    ...
    torch.save(net.state_dict(), f"models/cnn-parrallel-{parrallel}.pth")
    ...
    plt.legend()
    plt.savefig(f"results/Loss-parrallel-{parrallel}.png")
    plt.show()
    ...
    plt.savefig(f"results/Accuracy-parrallel-{parrallel}.png")
    plt.show()

    return train_time, total_accuracy
...
if __name__ == "__main__":
    ...
    trainloader, testloader, classes = parse_data()
    time1, accuracy1 = run(False, trainloader, testloader, classes)
    time2, accuracy2 = run(True, trainloader, testloader, classes)

    # Plotting the training time and accuracy in a bar chart with two subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 10))

    # Plotting training time
    ax1.bar(["Single GPU", "Multiple GPUs"], [time1, time2], color=['blue', 'orange'])
    ax1.set_xlabel("Configuration")
    ax1.set_ylabel("Training Time (s)")
    ax1.set_title("Training Time Comparison")

    # Plotting accuracy
    ax2.bar(["Single GPU", "Multiple GPUs"], [accuracy1, accuracy2], color=['blue', 'orange'])
    ax2.set_xlabel("Configuration")
    ax2.set_ylabel("Accuracy (%)")
    ax2.set_title("Accuracy Comparison")

    plt.tight_layout()
    plt.savefig("results/Time_Accuracy_Comparison.png")
    plt.show()