...
class CNN:
    def __init__(self):
        conv1 = Tensor(tf([3, 1, 3, 3], "gpu").random(0,1).mults(1 / np.sqrt(3*3*1)))
        conv2 = Tensor(tf([6, 3, 3, 3], "gpu").random(0,1).mults(1 / np.sqrt(3*3*3)))
        w1 = Tensor(tf([6*7*7, 84], "gpu").random(0,1).mults(1 / np.sqrt(84)))
        b1 = Tensor(tf([84],"gpu").random(0,1).mults(1 / np.sqrt(84)))
        w2 = Tensor(tf([84, 10], "gpu").random(0,1).mults(1 / np.sqrt(10)))
        b2 = Tensor(tf([10],"gpu").random(0,1).mults(1 / np.sqrt(10)))
        self.weights = [conv1, conv2, w1, b1, w2, b2]

    def forward(self, X, y):
        X = relu(conv(X, weight=self.weights[0], pad_h=1, pad_w=1))
        X = maxpool(X)
        X = relu(conv(X, weight=self.weights[1], pad_h=1, pad_w=1))
        X = maxpool(X)
        X = reshape(X, (X.shape[0], -1))
        X = relu(fc(X, weight=self.weights[2], bias=self.weights[3]))
        X = fc(X, weight=self.weights[4], bias=self.weights[5])
